...
def display_api(username, search_query):
    ...
    def fetch_random_videos(mongo_collection, limit=10):
        # Fetch random videos from MongoDB
        random_videos = mongo_collection.aggregate([
            {"$sample": {"size": limit}}
        ])
    ...
    def display_random_results(result):
...
# Main function
    ...
    for c in temp:
        if document_count <= 9:
            ...
            mongo_result = list(mongo_result)[0:10]
            break
    ...
    neo4j_count = 0
    temp1= user_preference_videos
    for c in temp1:
        neo4j_count+=1


    print(f"Number of user prefereed videos: {neo4j_count}")

    display_search_results(user_preference_videos)
    # print(len(DISPLAY_LIST),DISPLAY_LIST)

    n1 = 10 - (neo4j_count+document_count)
    print("Random to be displayed",n1)

    print("n+document_count",n+document_count)
    if n1>0:
        print("Random results")
        display_random_results(fetch_random_videos(mongo_collection, limit=n1))
    # Close connections
    mongo_client.close()
    return DISPLAY_LIST
def main():
    ...
    dis = display_api(username,search_query)
    # print(type(dis[0]))
